	...
	public void RunSimulation() {
		...
		for (int i = 0; i < numTrial; i++) {
			...
			for (int j = 0; j < Parameter.NO_FIVE_MIN_PER_HR; j++) {
				...
				if (Float.isInfinite(path[i][3][j])) {
					System.out.println(tempState.getR() + "_" + tempState.getG() + "_" + tempState.getPE());
					System.out.println(R);
					System.out.println(lmp[i][j]);
					System.out.println(
							EBState.getActionSpace()[tempState.getFeasibleActions().get(tempState.getOptAction(j))][0]
									+ "_" + EBState.getActionSpace()[tempState.getFeasibleActions()
											.get(tempState.getOptAction(j))][1]);
					System.out
							.println(((R - tempState.getR()) > 0 ? 1 : -1 * param.getBatteryParam()[Parameter.ETA_D]));
