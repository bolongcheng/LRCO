...
log_base = 1.0001;
b = log(sample_VF)./log(log_base);
a_b = LeastSq * b;  % a_b is the predicted values for all submatrices

f_part = log_base.^(a_b(1 : size_row,:)); %x vector for submatrix
g_part = log_base.^(a_b(size_row+1 : num_elements_part,:));%y vector for submatrix
